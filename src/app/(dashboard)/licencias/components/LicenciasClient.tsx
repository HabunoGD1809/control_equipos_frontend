...
export function LicenciasClient({
   ...
   const [modal, setModal] = useState<{
      type: ModalType;
      data?: LicenciaSoftware | SoftwareCatalogo;
   }>({ type: null });
   ...
   const [localCatalogo, setLocalCatalogo] =
      useState<SoftwareCatalogo[]>(initialCatalogo);
   ...
   useEffect(() => {
      if (showInactiveCatalogo && !hasFetchedAllCatalogo) {
         api
            .get<SoftwareCatalogo[]>("/licencias/software/", {
   ...
   const filteredCatalogo = useMemo(() => {
      return localCatalogo.filter(
         (c) => showInactiveCatalogo || c.is_active !== false,
      );
   }, [localCatalogo, showInactiveCatalogo]);

   const handleOpenModal = (
      type: ModalType,
      data: LicenciaSoftware | SoftwareCatalogo | null = null,
   ) => {
   ...
   const licenciasColumns: ColumnDef<LicenciaSoftware>[] = [
      {
         accessorFn: (row) =>
            `${row.software_info.nombre} ${row.software_info.version || ""}`,
         id: "software",
         header: "Software",
         cell: ({ row }) => (
            <span className="font-semibold text-foreground">
               {row.getValue("software")}
            </span>
         ),
      },
      {
            ...
            <DropdownMenu>
               ...
               <DropdownMenuContent align="end" className="w-48 shadow-md">
                  ...
                  {canAssignLicencias && row.original.cantidad_disponible > 0 && (
                     <DropdownMenuItem
                        onClick={() => handleOpenModal("asignar", row.original)}
                  ...
                  {canManageLicencias && (
                     <>
                        <DropdownMenuItem
                           onClick={() => handleOpenModal("licencia", row.original)}
                           className="cursor-pointer"
                        >
                        ...
                        <DropdownMenuItem
                           ...
                           onClick={() => licenciaDelete.openAlert(row.original.id)}
                        >
   ...
   const catalogoColumns: ColumnDef<SoftwareCatalogo>[] = [
      ...
      },
      {
         ...
         cell: ({ row }) => (
            <span className="text-muted-foreground">
               {row.original.marca_rel?.nombre || "N/A"}
            </span>
         ),
      },
      {
         ...
         cell: ({ row }) => (
            <span className="capitalize">{row.original.tipo_licencia}</span>
         ),
      },
      {
         accessorKey: "metrica_licenciamiento",
         header: "Métrica",
         cell: ({ row }) => (
            <span className="bg-muted px-2 py-1 rounded-md text-xs">
               {row.original.metrica_licenciamiento}
            </span>
         ),
      },
      {
            ...
            <DropdownMenu>
               ...
               <DropdownMenuContent align="end" className="w-40 shadow-md">
                  ...
                  {canManageCatalogo && (
                     <>
                        <DropdownMenuItem
                           onClick={() => handleOpenModal("catalogo", row.original)}
                           className="cursor-pointer"
                        >
                        ...
                        {row.original.is_active !== false && (
                           <DropdownMenuItem
                              className="text-destructive focus:text-destructive focus:bg-destructive/10 cursor-pointer"
                              onClick={() => catalogoDelete.openAlert(row.original.id)}
                           >
         ...
         >
            <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
               <DialogHeader>
                  ...
                  <DialogDescription>
                     {modal.type === "catalogo"
                        ? "Gestiona los tipos de software disponibles y sus métricas."
                        : modal.type === "licencia"
                           ? "Registre las licencias adquiridas y sus vencimientos."
                           : "Asigna una unidad de esta licencia a un empleado o estación de trabajo."}
                  </DialogDescription>
               </DialogHeader>
               {modal.type === "catalogo" && (
                  <SoftwareCatalogoForm
                     initialData={modal.data as SoftwareCatalogo}
                     onSuccess={handleCloseModal}
                  />
               )}
               {modal.type === "licencia" && (
                  <LicenciaSoftwareForm
                     initialData={modal.data as LicenciaSoftware}
                     catalogo={initialCatalogo.filter((c) => c.is_active !== false)}
               ...
               {modal.type === "asignar" && (
                  <AsignarLicenciaForm
                     licenciaId={(modal.data as LicenciaSoftware).id}
                     equipos={equipos}
                     usuarios={usuarios}
                     onSuccess={handleCloseModal}
                  />
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
               ...
               <div className="flex gap-2 w-full sm:w-auto items-center">
                  ...
                  {activeTab === "licencias" && canManageLicencias && (
                     <Button
                        onClick={() => handleOpenModal("licencia")}
               ...
               <TabsContent value="catalogo" className="mt-0 outline-none">
                  <div className="flex justify-end mb-3">
                     <div className="flex items-center space-x-2">
                        <Switch
                           id="show-inactive-catalogo"
                           checked={showInactiveCatalogo}
                           onCheckedChange={setShowInactiveCatalogo}
                        />
                        <Label
                           htmlFor="show-inactive-catalogo"
