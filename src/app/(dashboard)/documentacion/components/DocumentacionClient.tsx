...
import { VerifyDocumentoForm } from "@/components/features/documentos/VerifyDocumentoForm";
...
export function DocumentacionClient({ initialData }: DocumentacionClientProps) {
   ...
   const columns: ColumnDef<Documentacion>[] = [
      ...
      {
         accessorFn: (row) => row.equipo?.nombre || "N/A",
         id: "equipo",
         header: "Equipo Asociado",
         cell: ({ row }) =>
            row.original.equipo ? (
               <Link
                  href={`/equipos/${row.original.equipo.id}`}
   ...
   return (
      <>
         ...
         <DataTable columns={columns} data={initialData} filterColumn="titulo" />
      </>
   );
}
