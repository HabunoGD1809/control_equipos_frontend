...
type CheckInFormValues = z.infer<typeof reservaCheckInOutSchema>;
...
export function CheckInModal({ reserva, isOpen, onClose, onSuccess }: CheckInModalProps) {
   ...
   const form = useForm<CheckInFormValues>({
      resolver: standardSchemaResolver(reservaCheckInOutSchema),
      defaultValues: {
         notas_devolucion: "",
      },
   });
   ...
   const mutation = useMutation({
      ...
      onSuccess: () => {
         toast({ title: "Devolución Registrada", description: "El equipo ha sido marcado como devuelto (Check-In)." });
   ...
   const onSubmit = (data: CheckInFormValues) => mutation.mutate(data);

   return (
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
         <DialogContent className="sm:max-w-md">
            <DialogHeader>
               <DialogTitle className="flex items-center gap-2"><LogIn className="h-5 w-5 text-emerald-600" />Registrar Devolución (Check-In)</DialogTitle>
               <DialogDescription>
            ...
            <Form {...form}>
               <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2">
                  <FormField control={form.control} name="notas_devolucion" render={({ field }) => (
                     <FormItem>
                        <FormLabel>Notas de Devolución / Estado Final (Opcional)</FormLabel>
                        <FormControl>
                           <Textarea placeholder="Especifique si faltan cables o hay daños." className="resize-none min-h-25" {...field} value={field.value ?? ""} />
                        </FormControl>
                        <FormMessage />
                     </FormItem>
                  )}
